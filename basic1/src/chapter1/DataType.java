...
public class DataType {
	...
	public static void main(String[] args) {
		...
		System.out.println(double1); // 3.1415926535897930
		...
		System.out.println(numbers); // [I@4ae82894 주소 위치
		System.out.println(numbers2); // ''
		System.out.println(numbers3); // ''
		
		// 배열의 요소에 접근할 때는 '인덱스' 사용
		// 인덱스는 0부터 시작, 마지막 인덱스는 길이의 -1
		int item = numbers3[2];
		...
		// 배열의 길이를 확인하고자 할 땐 .length를 사용
		System.out.println(numbers3.length); // 5
		
		int[] example1 = new int[] {1, 2, 3};
		int[] example2 = example1;
		
		System.out.println(example1); // [I@1da51a35
		System.out.println(example2); // [I@1da51a35 두 개 동일 같은 주소값 참조
		
		example1[1] = 99;
		System.out.println(example1[1]); // 99
		System.out.println(example2[1]); // 99
		
		char[] chars1 = {'a', 'p', 'p', 'l', 'e'};
		System.out.println(chars1); // apple
		...
		string1 = new String();
		string1 = "apple";
		System.out.println(string1); //apple
		...
		System.out.println(statement + "@@@"); //   My name is loki  @@@
		System.out.println(statement.concat("!!!")); //   My name is loki  !!!
		
		// 2. 문자열 비교
		// equals(문자열) : 두 문자열이 동등한지 비교
		// compareTo(문자열) : 두 문자열을 사전순으로 비교 (대소문자 구분)
		// compareToIgnoreCase(문자열) : 두 문자열을 사전순으로 비교 (대소문자 구분X)
		System.out.println(statement.equals("My name is loki")); // false
		System.out.println(statement.compareTo("  My name is loki  ")); // A => 12 / Z => -13 / 똑같으면 0 : 앞에 있으면 양수, 뒤에 있으면 음수
		
		// 3. 문자열 길이
		// length() : 문자열의 길이를 반환
		System.out.println(statement.length()); // 19
		
		// 4. 문자열 변환
		// toUpperCase() : 모든 문자를 대문자로 변경
		// toLowerCase() : 모든 문자를 소문자로 변경
		// trim() : 문자열의 앞뒤 공백을 제거
		System.out.println(statement.toUpperCase()); //   MY NAME IS LOKI  
		System.out.println(statement.toLowerCase()); //   my name is loki  
		System.out.println(statement.trim()); // My name is loki (공백 사라짐)
		...
		System.out.println(statement.indexOf("기린")); // 6
		System.out.println(statement.indexOf("사자")); // -1 : 존재하지 않는 문자열
		System.out.println(statement.lastIndexOf("기린")); // 45
		System.out.println(statement.lastIndexOf("사자")); // -1 : 존재하지 않는 문자열
		
		// 6. 부분 문자열
		// substring(시작인덱스), substring(시작인덱스, 종료인덱스) : 문자열을 원하는 크기만큼 추출
		System.out.println(statement.substring(26)); // 네가 그린 기린 그림은 잘 못그린 기린 그림이다.
		System.out.println(statement.substring(26, 31)); // 네가 그린
